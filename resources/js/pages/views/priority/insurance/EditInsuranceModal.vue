...
const form = reactive({
    company_priority: props.insurance.company_priority,
    date_priority: props.insurance.date_priority,
    document_photo: props.insurance.document_photo,
    vehicle_id: props.insurance.vehicle_id,
})
...
const changePhoto = (e) => {
    ...
    props.insurance.document_photo = file;
}

const fetchVehiclesWithoutInsurance = async () => {
    const response = await axios.get('/api/fetch_vehicles_without_insurance')
...
const editInsurance = async () => {
    ...
    formData.append("date_priority", props.insurance.date_priority);
    formData.append("company_priority", props.insurance.company_priority);
    formData.append("vehicle_id", props.insurance.vehicle_id);

    if (props.insurance.document_photo) {
        formData.append("document_photo", props.insurance.document_photo);
    }
    await axios.post("/api/update_insurance/" + props.insurance.id, formData).then((response) => {
        if (response.data.success) {
            isLoading.value = false; // Désactivation du chargement
            window.location.reload();
            toast.fire({
                icon: "success",
                title: "Assurance editée avec succès",
            });
...
onMounted (async() => {
  await  fetchVehiclesWithoutInsurance()
})
        ...
        <div data-tw-merge class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-16 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] dark:bg-darkmode-600 sm:w-[460px]">
            ...
            <div data-tw-merge class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 sm:col-span-12">
                    ...
                    <select data-tw-merge id="modal-form-6" v-model="props.insurance.vehicle_id" class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1">
