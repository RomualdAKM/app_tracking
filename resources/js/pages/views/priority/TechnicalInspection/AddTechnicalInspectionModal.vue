...
const saveTechnicalinspection = async () => {
    isLoading.value = true; // Activation du chargement
    const formData = new FormData();
    formData.append("date_priority", form.date_priority);
    formData.append("company_priority", form.company_priority);
    formData.append("vehicle_id", form.vehicle_id);

    if (form.document_photo) {
        formData.append("document_photo", form.document_photo);
    }
    await axios.post("/api/save_technicalinspection", formData).then((response) => {
        if (response.data.success) {
            isLoading.value = false; // Désactivation du chargement
            window.location.reload();
            toast.fire({
                icon: "success",
                title: "Visite technique ajoutée avec succès",
            });
