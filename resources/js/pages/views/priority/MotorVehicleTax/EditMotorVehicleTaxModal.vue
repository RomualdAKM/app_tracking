...
const form = reactive({
    company_priority: props.MotorVehicleTax.company_priority,
    date_priority: props.MotorVehicleTax.date_priority,
    document_photo: props.MotorVehicleTax.document_photo,
    vehicle_id: props.MotorVehicleTax.vehicle_id,
})
...
const changePhoto = (e) => {
    ...
    props.MotorVehicleTax.document_photo = file;
}

const fetchVehiclesWithoutMotorVehicleTax = async () => {
    const response = await axios.get('/api/fetch_vehicles_without_MotorVehicleTax')
...
const editMotorVehicleTax = async () => {
    isLoading.value = true; // Activation du chargement
    const formData = new FormData();
    formData.append("date_priority", props.MotorVehicleTax.date_priority);
    formData.append("company_priority", props.MotorVehicleTax.company_priority);
    formData.append("vehicle_id", props.MotorVehicleTax.vehicle_id);

    if (props.MotorVehicleTax.document_photo) {
        formData.append("document_photo", props.MotorVehicleTax.document_photo);
    }
    await axios.post("/api/update_MotorVehicleTax/" + props.MotorVehicleTax.id, formData).then((response) => {
...
onMounted (async() => {
  await  fetchVehiclesWithoutMotorVehicleTax()
})
...
<template>
    <!-- BEGIN: Modal Content -->
    <div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="edit-MotorVehicleTax-modal-preview" class="modal group bg-gradient-to-b from-theme-1/50 via-theme-2/50 to-black/50 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
        <div data-tw-merge class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-16 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] dark:bg-darkmode-600 sm:w-[460px]">
            ...
            <div data-tw-merge class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 sm:col-span-12">
                    ...
                    <select data-tw-merge id="modal-form-6" v-model="props.MotorVehicleTax.vehicle_id" class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1">
