...
const form = reactive({
    name_type_breakdown: props.typebreakdown.name_type_breakdown,
    description_type_breakdown: props.typebreakdown.description_type_breakdown,
})

const isLoading = ref(false);

const editTypeBreakdown = async () => {

    isLoading.value = true; // Activation du chargement
    const formData = new FormData();
    formData.append("name_type_breakdown", props.typebreakdown.name_type_breakdown);
    formData.append("description_type_breakdown", props.typebreakdown.description_type_breakdown);
  
    await axios.post("/api/update_type_breakdown/"+  props.typebreakdown.id , formData).then((response) => {
...
<template>
     <!-- BEGIN: Modal Content -->
     <div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="edit-typebreakdown-modal-preview" class="modal group bg-gradient-to-b from-theme-1/50 via-theme-2/50 to-black/50 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
        <div data-tw-merge class="w-[90%] mx-auto bg-white relative rounded-md shadow-md transition-[margin-top,transform] duration-[0.4s,0.3s] -mt-16 group-[.show]:mt-16 group-[.modal-static]:scale-[1.05] dark:bg-darkmode-600 sm:w-[460px]">
            ...
            <div data-tw-merge class="p-5 grid grid-cols-12 gap-4 gap-y-3">
                ...
                <div class="col-span-12 sm:col-span-12">
                    ...
                    <textarea id="modal-form-2" v-model="props.typebreakdown.description_type_breakdown" placeholder="Description" class="disabled:bg-slate-100 disabled:cursor-not-allowed dark:disabled:bg-darkmode-800/50 dark:disabled:border-transparent [&[readonly]]:bg-slate-100 [&[readonly]]:cursor-not-allowed [&[readonly]]:dark:bg-darkmode-800/50 [&[readonly]]:dark:border-transparent transition duration-200 ease-in-out w-full h-24 resize-none text-sm border-slate-200 shadow-sm rounded-md placeholder:text-slate-400/90 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 dark:placeholder:text-slate-500/80"></textarea>
                </div>     
