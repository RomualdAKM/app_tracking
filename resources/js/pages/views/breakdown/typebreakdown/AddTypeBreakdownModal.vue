...
const form = reactive({
    name_type_breakdown: "",
    description_type_breakdown: "",
})

const isLoading = ref(false);

const saveTypeBreakdown = async () => {

    isLoading.value = true; // Activation du chargement
  
    await axios.post("/api/save_type_breakdown", form).then((response) => {
        if (response.data.success) {
            isLoading.value = false; // Désactivation du chargement
            window.location.reload();
            toast.fire({
                icon: "success",
                title: "Type de panne ajoutée avec succès",
            });
...
<template>
     <!-- BEGIN: Modal Content -->
     <div data-tw-backdrop="" aria-hidden="true" tabindex="-1" id="add-typebreakdown-modal-preview" class="modal group bg-gradient-to-b from-theme-1/50 via-theme-2/50 to-black/50 transition-[visibility,opacity] w-screen h-screen fixed left-0 top-0 [&:not(.show)]:duration-[0s,0.2s] [&:not(.show)]:delay-[0.2s,0s] [&:not(.show)]:invisible [&:not(.show)]:opacity-0 [&.show]:visible [&.show]:opacity-100 [&.show]:duration-[0s,0.4s]">
